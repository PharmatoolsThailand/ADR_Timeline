...
// ==========================================
// ระบบประเมิน Naranjo Algorithm
// ==========================================
function openNaranjo() {
    document.getElementById('naranjoModal').style.display = 'flex';
    calculateNaranjo(); // สั่งให้อัปเดตคะแนนตอนเปิด
}

function closeNaranjo() {
    document.getElementById('naranjoModal').style.display = 'none';
}

function selectNaranjo(groupId, btnElement, value) {
    const group = document.getElementById(groupId);
    const buttons = group.querySelectorAll('.nara-btn');

    // ล้างสีปุ่มเดิม และใส่สีปุ่มใหม่
    buttons.forEach(btn => btn.classList.remove('active'));
    btnElement.classList.add('active');

    // บันทึกคะแนนไว้ใน HTML Attribute เพื่อให้จำค่าได้
...
function calculateNaranjo() {
    ...
    // วนเช็คทั้ง 10 ข้อว่าข้อไหนมี data-val (ถูกกดเลือกแล้ว) บ้าง
    for(let i=1; i<=10; i++) {
        const group = document.getElementById('nara-g' + i);
        if(group && group.hasAttribute('data-val')) {
            total += parseInt(group.getAttribute('data-val'));
            answered++;
        }
    }

    document.getElementById('naranjoTotal').innerText = `คะแนนรวม: ${total}`;

    let level = "-";
    let color = "#475569";

    if (answered > 0) {
        if (total >= 9) { level = "Definite (ใช่แน่)"; color = "#b91c1c"; }
        else if (total >= 5) { level = "Probable (น่าจะใช่)"; color = "#ea580c"; }
        else if (total >= 1) { level = "Possible (อาจจะใช่)"; color = "#ca8a04"; }
        else { level = "Doubtful (น่าสงสัย)"; color = "#475569"; }
    }
...
function copyNaranjoToNote() {
    let answered = 0;
    for(let i=1; i<=10; i++) {
        if(document.getElementById('nara-g' + i).hasAttribute('data-val')) answered++;
    }
    if (answered === 0) return alert("กรุณาประเมินอย่างน้อย 1 ข้อครับ");

    const total = document.getElementById('naranjoTotal').innerText.replace('คะแนนรวม: ', '');
    const levelStr = document.getElementById('naranjoLevel').innerText.replace('ระดับความน่าจะเป็น: ', '');
    const noteEl = document.getElementById('pharmaNote');

    let textToAdd = `\n[การประเมิน Naranjo Algorithm]: ได้ ${total} คะแนน (${levelStr})`;
